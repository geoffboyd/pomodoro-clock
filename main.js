...
function startTimer() {					// starts the timer
	timerStarted = true;
	timerType = "session";
	document.getElementById("countdown-type").innerHTML = "Session";
	if (document.getElementById("session-time").innerHTML.length <= 2){
		var time = Number(document.getElementById("session-time").innerHTML);
		time -= 1;
...
function startBreakTimer() {					// starts the timer
	timerStarted = true;
	timerType = "break";
	document.getElementById("countdown-type").innerHTML = "Break!";
	if (document.getElementById("break-time").innerHTML.length <= 2){
		var time = Number(document.getElementById("break-time").innerHTML);
		time -= 1;
		document.getElementById("timer-countdown").innerHTML = '<div id="minutes-tens" class="unselectable inline-block">' + Math.floor(time/10) + '</div><div id="minutes-ones" class="unselectable inline-block">' + (time % 10) + '</div>:<div id="seconds-tens" class="unselectable inline-block">5</div><div id="seconds-ones" class="unselectable inline-block">9</div>'
	}
	tickTock();
